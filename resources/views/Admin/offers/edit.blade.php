...
@endpush
@section('breadcrumb')
    {{ Breadcrumbs::render('offers.edit', $offer) }}
@endsection
@section('content')
    <div class="col-12">
    ...
    <div class="col-xl-12 mb-30">
        <form method="post" action="{{ route('offers.update', $offer->id) }}" enctype="multipart/form-data" id="example-form">
            <div>
                <h3>بيانات العرض</h3>
                @csrf
                @method('PUT')
                <div class="form-row">
                    <div class="col-6">
                        ...
                        <input type="text" name="title[ar]" value="{{ $offer->getTranslation('title', 'ar') }}"
                            class="form-control" id="title.ar">
                    </div>
                    <div class="col-6">
                        <label for="title.en">العنوان بالانجليزية </label>
                        <input type="text" name="title[en]" value="{{ $offer->getTranslation('title', 'en') }}"
                            class="form-control" id="title.en">
                    </div>
                </div>
                <div class="form-row">
                    <div class="col-4">
                    ...
                    <div class="col-4">
                        ...
                        <input type="datetime-local" name="start_at" value="{{ $offer->start_at }}" class="form-control"
                            id="start_at">
                    </div>
                    <div class="col-4">
                        <label for="end_at"> تاريخ انتهاء العرض </label>
                        <input type="datetime-local" name="end_at" value="{{ $offer->end_at }}" class="form-control"
                            id="end_at">
                    </div>
                </div>
                <div class="form-row">
                    <div class="col-12">
                        ...
                        <textarea name="description[ar]" class="form-control" id="descriptionar">{{ $offer->getTranslation('description', 'ar') }}</textarea>
                    </div>
                    <div class="col-12">
                        <label for="descriptionen"> التفاصيل بالانجليزية </label>
                        <textarea name="description[en]" class="form-control" id="descriptionen">{{ $offer->getTranslation('description', 'en') }}</textarea>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <select name="status" class="custom-select" id="status">
                        @foreach ($statuses as $status => $value)
                            <option @selected($offer->status == $value) value="{{ $value }}">
                                {{ $status }}
                            </option>
                        @endforeach
                    </select>
                        ...
                        <div class="col-3">
                            ...
                            <label for="inputGroupFile01">
                                <img for="inputGroupFile01" id="image"
                                    src="{{ asset($offer->getFirstMediaUrl('offers')) }}" class="w-100"
                                    alt="{{ $offer->title }}" style="cursor: pointer">
                            </label>
                ...
                <div class="form-group">
                    <div class="repeater">
                        <div data-repeater-list="products">
                            @forelse ($offer->products as $offer_product)
                                    ...
                                    <div class="row" id="product">
                                        <div class="col-6">
                                            ...
                                            <select name="product_id" class="form-control select2">
                                                ...
                                                @foreach ($products as $product)
                                                    <option @selected($offer_product->pivot->product_id == $product->id) value="{{ $product->id }}">
                                        ...
                                        <div class="col-3">
                                            ...
                                            <input type="number" name="discount" class="form-control specValue"
                                                id="product_discount" value="{{ $offer_product->pivot->discount }}">
                                        </div>
                                        <div class="col-lg-3">
